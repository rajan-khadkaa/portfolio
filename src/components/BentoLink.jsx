...
const BentoLink = ({ github, demo, hover, gitlink, playVidIcon }) => {
  return (
    <div className="top-icons-div  flex gap-2 justify-center items-center absolute z-20 p-3 bg-black right-0 top-0 rounded-bl-4xl  ">
      ...
      <a
        className={`p-2 cursor-pointer ${!playVidIcon && "rotate-45"} ${
          playVidIcon && ""
        } rounded-full bg-white-50 text-black flex items-center justify-center`}
        href={demo}
        target="_blank"
      >
        {playVidIcon ? (
          <FaPlay
            // className={`${
            //   hover && "animate-ping -rotate-45 border-2 border-red-700"
            // }`}
